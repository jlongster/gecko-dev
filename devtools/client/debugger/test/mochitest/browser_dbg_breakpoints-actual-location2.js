...
function test() {
  initDebugger(TAB_URL).then(([aTab,, aPanel]) => {
    ...
    Task.spawn(function*() {
      ...
      const movedBpState = yield actions.addBreakpoint({
        actor: gSources.selectedValue,
        line: 17
      });

      is(movedBpState.breakpoint.location.actor, gSources.selectedValue,
         "Breakpoint client url is the same.");
      is(movedBpState.value.actualLocation.line, 19,
