...
function test() {
  initDebugger(TAB_URL).then(([aTab,, aPanel]) => {
    ...
    Task.spawn(function*() {
      ...
      const bpState = yield actions.addBreakpoint({
        actor: gSources.selectedValue, line: 4
      });

      is(bpState.breakpoint.location.actor, gSources.selectedValue, "has the right source");
      ok(bpState.value.actualLocation, "has an actualLocation");
      is(bpState.value.actualLocation.line, 6, "moved to line 6");
