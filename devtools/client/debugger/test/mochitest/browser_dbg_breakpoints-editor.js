...
function test() {
  initDebugger(TAB_URL).then(([aTab,, aPanel]) => {
    ...
    const actions = makeActionsThatWait(gPanel);
    ...
    Task.spawn(function*() {
      ...
      yield actions.selectSource(gSources.items[0].attachment.source);

      executeSoon(() => gDebugger.requestAnimationFrame(onReadyForClick));
    ...
    function checkFirstBreakpoint(bpState) {
      breakpointsAdded++;
      const bp = bpState.breakpoint;
      ...
      ok(queries.getBreakpoint(getState(), bp.location),
         "breakpoint1 found in the list of added breakpoints.");
    ...
    function checkFirstBreakpointRemoved(bpState) {
      breakpointsRemoved++;
      const bp = bpState.breakpoint;
    ...
    function testBreakpointAddBackground() {
      ...
      return actions.addBreakpoint(location);
    }
    ...
    function checkBackgroundBreakpoint(bpState) {
      breakpointsAdded++;
      const bp = bpState.breakpoint;
