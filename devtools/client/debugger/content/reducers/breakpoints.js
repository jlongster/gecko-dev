  ...
  case constants.ADD_BREAKPOINT: {
    ...
    if(action.status === 'start') {
      ...
      state = setIn(state, ['breakpoints', id], bp.merge({
        disabled: false,
        loading: true,
        condition: 'condition' in action ? action.condition : undefined
      }));

      emit(existingBp ? "breakpoint-enabled" : "breakpoint-added",
      ...
      emit('breakpoint-updated', state.breakpoints[finalLocationId]);
      return state;
    }
    else if(action.status === 'error') {
      // Remove the optimistic update
      emit('breakpoint-removed', state.breakpoints[id]);
  ...
  case constants.REMOVE_BREAKPOINT: {
    if(action.status === 'done') {
      ...
      if(action.disabled) {
        ...
        emit('breakpoint-disabled', state.breakpoints[id]);
        return state;
      }

      state = deleteIn(state, ['breakpoints', id]);
      emit('breakpoint-removed', bp);
      return state;
    }
    break;
  }
