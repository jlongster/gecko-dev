...
var DebuggerController = {
  ...
  initialize: function() {
    ...
    const store = createStore(combineEmittingReducers(
      reducers,
      emitter.emit
    ));
    emitter.bindStore(store);
    this.on = emitter.on;
  ...
  _settleAllRequests: function() {
    ...
    if(requests.openRequests.length > 0) {
      ...
      this.on('@redux:open-requests', reqs => {
        if(reqs.length === 0) {
          deferred.resolve();
        }
...
StackFrames.prototype = {
  ...
  _handleConditionalBreakpoint: Task.async(function*() {
    ...
    dump('JWL breakLocation ' + breakLocation + '\n');
    if (!breakLocation) {
      return;
    }
    ...
    dump('JWL condition: ' + conditionalExpression + '\n');
    ...
    dump('JWL: ' + this._currentEvaluation.return + '\n');
