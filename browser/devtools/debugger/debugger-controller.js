...
let DebuggerController = {
  ...
  _settleAllRequests: function() {
    const requests = dispatcher.getState().asyncRequests;

    if(requests.openRequests.length > 0) {
      const deferred = promise.defer();
      dispatcher.onChange({
        asyncRequests: { 'open-requests': reqs => {
          if(reqs.length === 0) {
            deferred.resolve();
          }
  ...
  connectThread: function() {
    const actions = bindActionCreators(stores.sources.actions,
                                       dispatcher.dispatch);
    // TODO: bug 806775, update the globals list using aPacket.hostAnnotations
    // from bug 801084.
    // this.client.addListener("newGlobal", ...);
    this.activeThread.addListener("newSource", actions.newSource);
  ...
  _onWillNavigate: function() {
    ...
    dispatcher.dispatch(actions.unload());
  ...
  _onNavigate: function() {
    ...
    dispatcher.dispatch(actions.loadSources());
  },
...
StackFrames.prototype = {
  ...
  _handleConditionalBreakpoint: Task.async(function*() {
    ...
    let bp = queries.getBreakpoint(dispatcher.getState(), breakLocation);
