...
BreakpointStore.prototype = {
  ...
  _iterLines: function* (actor, aLine) {
    if (aLine != null) {
      if ((this._wholeLineBreakpoints[actor]
           && this._wholeLineBreakpoints[actor][aLine])
          || (this._breakpoints[actor] && this._breakpoints[actor][aLine])) {
        yield aLine;
      }
    } else {
      const wholeLines = this._wholeLineBreakpoints[actor]
        ? Object.keys(this._wholeLineBreakpoints[actor])
        : [];
      const columnLines = this._breakpoints[actor]
        ? Object.keys(this._breakpoints[actor])
        : [];
  ...
  _iterColumns: function* (actor, aLine, aColumn) {
    if (!this._breakpoints[actor] || !this._breakpoints[actor][aLine]) {
      return;
    }

    if (aColumn != null) {
      if (this._breakpoints[actor][aLine][aColumn]) {
        yield aColumn;
      }
    } else {
      for (let column in this._breakpoints[actor][aLine]) {
        yield column;
      }
    }
  },
...
SourceActor.prototype = {
  ...
  _createAndStoreBreakpoint: function (aRequest) {
    let bp = merge(aRequest, { source: this.form() });
   ...
   *        location is in the generated source, if sourcemapped.
   */
  _setBreakpoint: function (aRequest) {
    let actor;
    let bp = merge(aRequest, { source: this.form() });
    let storedBp = this.breakpointStore.getBreakpoint(bp);
    if (storedBp.actor) {
      actor = storedBp.actor;
      actor.condition = bp.condition;
    } else {
    ...
    */
    ...
    for (let script of scripts) {
      this._findClosestOffsetMappings(bp,
                                      script,
    ...
    */
    ...
    for (let script of scripts) {
      let offsets = script.getAllOffsets();
      for (let line = bp.line; line < offsets.length; ++line) {
        if (offsets[line]) {
    ...
    if (found) {
      ...
      if (existingBp && existingBp.actor) {
         ...
         */
        actor.onDelete();
        this.breakpointStore.removeBreakpoint(bp);
        return {
   ...
   *        An object of the form { line[, column] }.
...
BreakpointActor.prototype = {
  ...
  onDelete: function (aRequest) {
    ...
    this.threadActor.breakpointStore.removeBreakpoint(
      merge(this.location, { source: this.location.sourceActor.form() })
    );
